...
public sealed class TicketsController(ITicketService tickets, ICurrentUser current) : ControllerBase
{
    ...
    [HttpGet] public Task<PagedResult<TicketListItem>> List([FromQuery] string scope = "all", [FromQuery] string? status = null, [FromQuery] string? search = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 30, CancellationToken ct = default) => tickets.ListAsync(scope, status, search, current.Id, page, pageSize, ct);
    [HttpGet("mine")] public Task<PagedResult<TicketListItem>> Mine([FromQuery] string? status = null, [FromQuery] string? search = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 30, CancellationToken ct = default) => tickets.ListAsync("mine", status, search, current.Id, page, pageSize, ct);
    [HttpGet("unassigned")] public Task<PagedResult<TicketListItem>> Unassigned([FromQuery] string? status = null, [FromQuery] string? search = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 30, CancellationToken ct = default) => tickets.ListAsync("unassigned", status, search, current.Id, page, pageSize, ct);
